...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    return Compose(
        [
            FillWithRandomScaledShiftedCrop(),
            pre_transform,
...
# class FillWithRandomScaledShiftedCrop:
#     """
#     Fills the areas inside the bounding boxes with a randomly scaled and shifted crop from the image,
#     and applies the same scale and shift to the filled area.
#     """

#     def __init__(self):
#         """
#         Initializes the FillWithRandomScaledShiftedCrop object.
#         """
#         pass

#     def __call__(self, labels):
#         """
#         Applies the fill with random scaled and shifted crop transformation to the input image inside the bounding boxes,
#         and applies the same scale and shift to the filled area.
...
#         Returns:
#             (dict): The same dict with the image where areas inside bounding boxes are filled with random scaled and shifted crops,
#                     and the filled areas are also scaled and shifted accordingly.
#         """
...
#         intersection_mask = np.zeros_like(background_mask)
...
#             # Random scale and shift factors
#             scale_factor = random.uniform(0.5, 0.9)
            
#             shift_pic_factor_x = random.uniform(-0.3, 0.3) * (w - bbox_width)
#             shift_pic_factor_y = random.uniform(-0.3, 0.3) * (h - bbox_height)

#             # Calculate new crop size and position
#             crop_width = int(scale_factor * bbox_width)
#             crop_height = int(scale_factor * bbox_height)
#             crop_x1 = max(0, min(w - crop_width, int(shift_pic_factor_x)))
#             crop_y1 = max(0, min(h - crop_height, int(shift_pic_factor_y)))

#             # Ensure the crop size is not larger than the original image size
#             crop_width = min(crop_width, w - crop_x1)
#             crop_height = min(crop_height, h - crop_y1)

#             if crop_width <= 0 or crop_height <= 0:
#                 continue
#             # print("crop_x1: ", crop_x1, "crop_y1: ", crop_y1, "crop_width: ", crop_width, "crop_height: ", crop_height)

#             # Randomly select a crop from the image
#             random_crop = img[crop_y1:crop_y1 + crop_height, crop_x1:crop_x1 + crop_width]


#             shift_factor_x = random.uniform(-0.3, 0.3) * bbox_width
#             shift_factor_y = random.uniform(-0.3, 0.3) * bbox_height

#             # Calculate the target position and size for the filled area
#             target_x1 = x1 + int(shift_factor_x)
#             target_y1 = y1 + int(shift_factor_y)
#             target_width = int(scale_factor * bbox_width)
#             target_height = int(scale_factor * bbox_height)

#             # Resize the random crop to the target size
#             if target_width <= 0 or target_height <= 0:
#                 continue
#             resized_crop = cv2.resize(random_crop, (target_width, target_height), interpolation=cv2.INTER_LINEAR)

#             # Ensure the target area is within image bounds
#             target_x1 = max(0, min(w - target_width, target_x1))
#             target_y1 = max(0, min(h - target_height, target_y1))
#             target_x2 = min(w, target_x1 + target_width)
#             target_y2 = min(h, target_y1 + target_height)

#             # print("target_x1: ", target_x1, "target_y1: ", target_y1, "target_width: ", target_width, "target_height: ", target_height)
#             # Fill the target area with the resized crop
#             if random.random() < 0.5:
#                 img[target_y1:target_y2, target_x1:target_x2] = 0
#             else:
#                 img[target_y1:target_y2, target_x1:target_x2] = resized_crop
            
#             target_x1 = int(random.uniform(0, w - target_width))
#             target_y1 = int(random.uniform(0, h - target_height))
#             target_x2 = min(w, target_x1 + target_width)
#             target_y2 = min(h, target_y1 + target_height)
#             intersection_mask[target_y1:target_y2, target_x1:target_x2] = True

#         # 部分非目标区域填充为黑色
#         final_mask = np.logical_and(background_mask, intersection_mask)
#         img[final_mask] = [0, 0, 0]

#         labels["img"] = img
#         labels["instances"] = instances  # Put instances back into labels if needed for further processing
#         return labels
...
class FillWithRandomScaledShiftedCrop:
    ...
    def bbox_intersects(self, bbox1, bbox2):
        # 检查两个检测框是否相交
        x1, y1, x2, y2 = bbox1
        x1_other, y1_other, x2_other, y2_other = bbox2

        # 如果一个检测框在另一个检测框的右侧或左侧
        if x1 > x2_other or x1_other > x2:
            return False
        # 如果一个检测框在另一个检测框的上方或下方
        if y1 > y2_other or y1_other > y2:
            return False
        return True
    
    def random_mask(self, mask, bboxes):
        ...
        if len(bboxes) > 0:
            ...
            avg_w = avg_w / len(bboxes)
            avg_h = avg_h / len(bboxes)

        # 随机生成10个填充区域
        for i in range(10):
            target_w = int(random.uniform(avg_w / 1.5, avg_w * 1.5))
            target_h = int(random.uniform(avg_h / 1.5, avg_h * 1.5))
            target_x1 = max(0, int(random.uniform(0, img_w - target_w)))
            target_y1 = max(0, int(random.uniform(0, img_h - target_h)))
            target_x2 = min(img_w, target_x1 + target_w)
            target_y2 = min(img_h, target_y1 + target_h)
            for other_bbox in bboxes:
                if not self.bbox_intersects([target_x1, target_y1, target_x2, target_y2], other_bbox):
                    mask[target_y1:target_y2, target_x1:target_x2] = True
                else:
                    pass

    def fill_bbox(self, img, bboxes):
        for bbox in bboxes:
            x1, y1, x2, y2 = map(int, bbox)
            img[y1:y2, x1:x2] = np.random.randint(0, 256, 3)
    
    def random_cross_mask(self, mask, img_w, img_h, bboxes):
        if random.random() < 0.1:
            return
        if len(bboxes) < 10:
            for i in range(10):
                if random.random() < 0.5:
                    ...
                    target_x1 = max(int(center_x - 3), 0)
                    target_x2 = min(int(center_x + 3), img_w)
                    target_y1 = 0
                    target_y2 = img_h
                else:
                    center_y = random.uniform(0, img_h)
                    target_y1 = max(int(center_y - 3), 0)
                    target_y2 = min(int(center_y + 3), img_h)
                    target_x1 = 0
                    target_x2 = img_w
                mask[target_y1:target_y2, target_x1:target_x2] = True
    
    def debug_output(self, filename, img, non_object_mask, random_crop_mask, final_mask ,bboxes):
        output_path = "D:\\temp\\debug_output\\"
        if not os.path.exists(output_path):
            os.makedirs(output_path)
        output_img_path = os.path.join(output_path, filename + "_img.jpg")
        output_non_object_mask_path = os.path.join(output_path, filename + "_non_object_mask.jpg")
        output_random_crop_mask_path = os.path.join(output_path, filename + "_random_crop_mask.jpg")
        output_final_mask_path = os.path.join(output_path, filename + "_final_mask.jpg")
        ...
        cv2.imwrite(output_non_object_mask_path, non_object_mask.astype(np.uint8) * 255)
        cv2.imwrite(output_random_crop_mask_path, random_crop_mask.astype(np.uint8) * 255)
        cv2.imwrite(output_final_mask_path, final_mask.astype(np.uint8) * 255)

    def __call__(self, labels):
        img = labels["img"]
        instances = labels.pop("instances")
        filename = os.path.splitext(os.path.basename(labels["im_file"]))[0]
        ...
        self.fill_bbox(background_img, instances.bboxes)

        non_object_mask = np.ones((h, w), dtype=bool)
        for bbox in instances.bboxes:
            x1, y1, x2, y2 = map(int, bbox)
            bw = x2 - x1
            bh = y2 - y1
            ...
            x2 = min(w, x2 + bw // 5)
            y2 = min(h, y2 + bh // 5)
            non_object_mask[y1:y2, x1:x2] = False

        # 对non_object_mask进行腐蚀操作
        kernel = np.ones((11, 11), np.uint8)
        non_object_mask = cv2.erode(non_object_mask.astype(np.uint8), kernel, iterations=1).astype(bool)

        random_crop_mask = np.zeros_like(non_object_mask)

        for i, bbox in enumerate(instances.bboxes):
            intersects = False
            for j, other_bbox in enumerate(instances.bboxes):
                if i != j and self.bbox_intersects(bbox, other_bbox):
                    intersects = True
                    break
            ...
            shift_factor_x = random.uniform(-0.2, 0.2) * bbox_width
            shift_factor_y = random.uniform(-0.2, 0.2) * bbox_height
            # shift_factor_x = 0
            # shift_factor_y = 0
            ...
            # Ensure the target area is within image bounds
            if random.random() < 0.5:
                img[target_y1:target_y2, target_x1:target_x2] = np.random.randint(0, 256, 3)
            else:
                src_x1 = random.randint(0, w - target_width)
                src_y1 = random.randint(0, h - target_height)
                src_x2 = src_x1 + target_width
                src_y2 = src_y1 + target_height
                crop_img = background_img[src_y1:src_y2, src_x1:src_x2]
                cv2.resize(crop_img, (target_width, target_height), img[target_y1:target_y2, target_x1:target_x2])

        
        self.random_mask(random_crop_mask, instances.bboxes)

        # 部分非目标区域填充为黑色
        final_mask = np.logical_and(non_object_mask, random_crop_mask)
        self.random_cross_mask(final_mask, w, h, instances.bboxes)
        img[final_mask] = np.random.randint(0, 256, 3)

        # self.debug_output(filename, img, non_object_mask, random_crop_mask, final_mask, instances.bboxes)
